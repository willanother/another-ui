...
type ClipPathWrapperProps = {
  path?: ReactNode;
} & PropsWithChildren;

function ClipPathWrapper({
  children,
  path = (
...
}: ClipPathWrapperProps) {
  const clipPathId = `clip-path-${uuidv4()}`;
  return (
    <>
      ...
      {/* children with clip path applied */}
      <figure style={{ clipPath: `url(#${clipPathId})` }}>{children}</figure>
    </>
  );
}

export { ClipPathWrapper, type ClipPathWrapperProps };
